'use client'

import useSWR, { useSWRConfig } from 'swr'
const fetcher = (...args) => fetch(...args).then(res => res.json())

export default function Home() {
    const { data, error, isLoading } = useSWR('http://localhost:8000/api/all_info', fetcher)
    if (isLoading) {
        return (
            <p>Loading...</p>
        )
    }
    return (
        <div className="grid grid-rows-2 grid-cols-2 gap-4 p-4">
            <div className="flex flex-col rounded-md border p-2">
                <div className="flex flex-row gap-2 items-center">
                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24"><path fill="currentColor" d="M5.6 16.8q1.125-.875 1.763-2.125T8 12t-.638-2.675T5.6 7.2q-.775 1.05-1.187 2.275T4 12t.413 2.525T5.6 16.8M12 20q1.375 0 2.663-.437T17 18.25q-1.425-1.15-2.212-2.787T14 12t.788-3.463T17 5.75q-1.05-.875-2.337-1.312T12 4t-2.662.438T7 5.75Q8.425 6.9 9.213 8.538T10 12t-.788 3.463T7 18.25q1.05.875 2.338 1.313T12 20m6.4-3.2q.775-1.05 1.188-2.275T20 12t-.413-2.525T18.4 7.2q-1.125.875-1.763 2.125T16 12t.638 2.675T18.4 16.8M12 22q-2.075 0-3.9-.788t-3.175-2.137T2.788 15.9T2 12t.788-3.9t2.137-3.175T8.1 2.788T12 2t3.9.788t3.175 2.137T21.213 8.1T22 12t-.788 3.9t-2.137 3.175t-3.175 2.138T12 22" /></svg>
                    <p className="text-sm font-medium text-center">
                        Shots Made
                    </p>
                </div>
                <p className="">
                    {data.shots_made} / {data.total_shots}
                </p>
            </div>
            <div className="flex flex-col rounded-md border p-2">
                <div className="flex flex-row gap-2 items-center">
                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24"><path fill="currentColor" d="M5.6 16.8q1.125-.875 1.763-2.125T8 12t-.638-2.675T5.6 7.2q-.775 1.05-1.187 2.275T4 12t.413 2.525T5.6 16.8M12 20q1.375 0 2.663-.437T17 18.25q-1.425-1.15-2.212-2.787T14 12t.788-3.463T17 5.75q-1.05-.875-2.337-1.312T12 4t-2.662.438T7 5.75Q8.425 6.9 9.213 8.538T10 12t-.788 3.463T7 18.25q1.05.875 2.338 1.313T12 20m6.4-3.2q.775-1.05 1.188-2.275T20 12t-.413-2.525T18.4 7.2q-1.125.875-1.763 2.125T16 12t.638 2.675T18.4 16.8M12 22q-2.075 0-3.9-.788t-3.175-2.137T2.788 15.9T2 12t.788-3.9t2.137-3.175T8.1 2.788T12 2t3.9.788t3.175 2.137T21.213 8.1T22 12t-.788 3.9t-2.137 3.175t-3.175 2.138T12 22" /></svg>
                    <p className="text-sm font-medium text-center">
                        Shots Missed
                    </p>
                </div>
                <p className="">
                    {data.shots_missed} / {data.total_shots}
                </p>
            </div>
            {/* <div className="flex flex-col rounded-md border p-2">
                <div className="flex flex-row gap-2 items-center">
                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24"><path fill="currentColor" d="M5.6 16.8q1.125-.875 1.763-2.125T8 12t-.638-2.675T5.6 7.2q-.775 1.05-1.187 2.275T4 12t.413 2.525T5.6 16.8M12 20q1.375 0 2.663-.437T17 18.25q-1.425-1.15-2.212-2.787T14 12t.788-3.463T17 5.75q-1.05-.875-2.337-1.312T12 4t-2.662.438T7 5.75Q8.425 6.9 9.213 8.538T10 12t-.788 3.463T7 18.25q1.05.875 2.338 1.313T12 20m6.4-3.2q.775-1.05 1.188-2.275T20 12t-.413-2.525T18.4 7.2q-1.125.875-1.763 2.125T16 12t.638 2.675T18.4 16.8M12 22q-2.075 0-3.9-.788t-3.175-2.137T2.788 15.9T2 12t.788-3.9t2.137-3.175T8.1 2.788T12 2t3.9.788t3.175 2.137T21.213 8.1T22 12t-.788 3.9t-2.137 3.175t-3.175 2.138T12 22" /></svg>
                    <p className="text-sm font-medium text-center">
                        Average shot
                    </p>
                </div>
                <p className="">
                    Overshooting
                </p>
            </div> */}
        </div>
    )
}